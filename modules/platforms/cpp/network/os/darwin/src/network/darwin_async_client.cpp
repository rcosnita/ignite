...
#include "network/darwin_async_client.h"
...
namespace ignite
{
    namespace network
    {
        ...
        bool LinuxAsyncClient::StartMonitoring(int epoll0)
        {
            ...
            struct kevent event;
            EV_SET(&event, fd, EVFILT_READ, EV_ADD | EV_EOF, 0, 0, this);

            int res = kevent(epoll0, &event, 1, NULL, 0, NULL);
            if (res < 0)
                return false;
        ...
        {
            struct kevent event;
            EV_SET(&event, fd, EVFILT_READ, EV_DELETE, 0, 0, this);
            kevent(epoll, &event, 1, NULL, 0, NULL);
        }

        void LinuxAsyncClient::EnableSendNotifications()
        {
            struct kevent event;
            EV_SET(&event, fd, EVFILT_WRITE, EV_ADD | EV_EOF, 0, 0, this);
            kevent(epoll, &event, 1, NULL, 0, NULL);
        }
